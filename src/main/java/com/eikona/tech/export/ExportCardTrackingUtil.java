...
@Component
public class ExportCardTrackingUtil {
	...
	public void fileExportBySearchValue(HttpServletResponse response, String cardId, String employee, String type,
			String sDate,String eDate, String flag) throws Exception {
		Date startDate = null;
		Date endDate = null;
		...
		List<CardTracking> cardManagementList = cardTrackingRepository.findAll(cardIdSpc.and(empSpc).and(typeSpc).and(dateSpec));
		
		generateExcel(response,cardManagementList);
	...
	private void generateExcel(HttpServletResponse response, List<CardTracking> cardManagementList) throws Exception {
			...
			String filename = "Card_Management_" + currentDateTime + ApplicationConstants.EXTENSION_EXCEL;
